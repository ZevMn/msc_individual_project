...
def run_padchest(model_to_evaluate, encoder_to_evaluate, shift):
    ...
    ### Create dataloaders
    val_df = pd.read_csv(Config.ROOT / "experiments" / "val_padchest.csv")
    test_df = pd.read_csv(Config.ROOT / "experiments" / "test_padchest.csv")
    ...
    task_output, mod_spec_encoder_output = get_or_save_outputs(
    ...
    match shift:
        case "no_shift":
            if encoder_to_evaluate == "simclr_imagenet":
                ...
                filename = f"outputs/new_padchest_prev_{prevalence_orig}_n{n_boostraps}_{model_id}_{encoder_id}_v{val_sizes[0]}_t{test_sizes}.csv"
        ...
        case "acquisition":
            ...
            for phillips_shifted in [0.25, 0.75, 1.0]:
                filename = f"outputs/new_padchest_acq_{phillips_shifted}_n{n_boostraps}_{model_id}_{encoder_id}_v{val_sizes[0]}_t{test_sizes}.csv"
        ...
        case "gender":
            ...
            for female_shifted in [0.25, 0.75, 1.0]:
                filename = f"outputs/new_padchest_gender_{female_shifted}_n{n_boostraps}_{model_id}_{encoder_id}_v{val_sizes[0]}_t{test_sizes}.csv"
        ...
        case "prevalence":
            ...
            for prevalence_shifted in [0.15, 0.20, 0.30]:
                filename = f"outputs/new_padchest_prev_{prevalence_shifted}_n{n_boostraps}_{model_id}_{encoder_id}_v{val_sizes[0]}_t{test_sizes}.csv"
        ...
        case "acquisition_prev":
            ...
            for phillips_shifted, prevalence_shifted in [
                ...
                filename = f"outputs/new_padchest_acq_prev_{phillips_shifted}_n{n_boostraps}_{model_id}_{encoder_id}_v{val_sizes[0]}_d{prevalence_shifted}_t{test_sizes}.csv"
